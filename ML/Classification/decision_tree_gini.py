...
def get_split(dataset):
    ...
    for index in range(len(dataset[0])-1):
        for row in dataset:
            # call the test_split function to get groups
            groups = test_split(index, row[index], dataset)
            # calculate the gini index using the gini_index function
            gini = gini_index(groups, class_values)
            if gini < b_score:
                # update best index, best value, best score and best groups
