...
def logistic_regression(X, y, num_iterations, learning_rate):
    ...
    for i in range(num_iterations):
        z = np.dot(X, theta)
        h = sigmoid(z)
        gradient = np.dot(X.T, (h - y)) / y.size # the derivative of cross entropy with sigmoid is x dot (pred - y)
...
def predict_prob(X, theta):
    # Add intercept to X
    intercept = np.ones((X.shape[0], 1))
    X = np.concatenate((intercept, X), axis=1) # or X = np.append(X, intercept, axis  = 1)
