# Given a number, you can perform only two types of operations with it:

# Add 1
# Multiply by 2
# Write a function min_steps(n) that takes an integer n and computes the minimum number of steps required to transform the number 0 into the number n.

# You should solve this task with 

# O(n) time complexity using dynamic programming.

# For example, min_steps(8) = 4, because:

# Start from 0
# Add 1 to get 1 (1 step)
# Multiply by 2 to get 2 (1 step)
# Multiply by 2 again to get 4 (1 step)
# Multiply by 2 one more time to get 8 (1 step)


import math
def min_steps(n, memo = {}):
    if n in memo:
        return memo[n]
    if n == 0:
        return 0
    if n == 1:
        return 1
    if n % 2 == 0:
        memo[n] = min_steps(n // 2, memo) + 1
    else:
        memo[n] = min_steps(n - 1, memo) + 1
    return memo[n]